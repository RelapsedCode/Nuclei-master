...
def ProcessImage(val):
    ...
    if morphOpenKernel != 0:
        ...
        openImg = mask_3
        mask_3 = cv.morphologyEx(mask_3, cv.MORPH_OPEN, (morphOpenKernel, morphOpenKernel))
    if morphCloseKernel != 0:
        morphCloseKernel = isEven(morphCloseKernel)
        mask_3 = cv.morphologyEx(mask_3, cv.MORPH_CLOSE, (morphCloseKernel, morphCloseKernel))

    cv2.imwrite(r"D:\side-projects\nuclei\results\default.jpg", mask_3)

    CountPixels(mask, hsv, mask_3)
    print('1 time 2 time')
...
def overlap():
   ...
   # Setting colors instead of masks
    img1_black[img1_black_pixels_mask] = [255, 0, 255] #Is it RGB/BGR?
    img1_black[img1_non_black_pixels_mask] = [255, 0, 0]

    img2_black[img2_black_pixels_mask] = [0, 255, 255] #Is it RGB?
